...
export async function POST(request) {
  ...
  if (pid === undefined || pswd === undefined) {
    console.log("here2");
    return NextResponse.json(
  ...
  try {
    const UserExists = await checkIfUserExists(pid);
    console.log("id exists bool: " + UserExists[0].pswd);

    if (UserExists.length > 0) {
      console.log("POST REQ CHECKING FOR id if block");
      try {
        const result = await new Promise((resolve, reject) => {
          bcrypt.compare(pswd, UserExists[0].pswd, (err, result) => {
            if (err) reject(err);
            else resolve(result);
          });
        });
        console.log(UserExists[0].p_name)
        if (result) {
          console.log(result);
          return NextResponse.json(
            { message: "LOGIN VALID",  name: UserExists[0].p_name},
            { status: 200 },
          );
        } else {
          return NextResponse.json(
